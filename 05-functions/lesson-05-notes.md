...
- **Closure**
  - Closure là một hàm ghi nhớ xung quanh nơi nó được sinh ra và gom vào một cái balo
  - Dù sau này hàm có mang đi để chạy chỗ khác thì nó vẫn mang cái balo đó đi dùng
  - Cú pháp: không có cú pháp riêng
...
function father(){
  ...
  - 3 dấu hiệu chính khi nhận biết 1 closure
    - hàm lồng trong hàm con nằm trong hàm cha
    - hàm con sử dụng biến của hàm cha
    - khi hàm con chạy nó vẫn truy cập được biến của hàm

This - 
This(tôi) là gì: bản chất của this là đại từ nhân xưng tôi trong đời sống
Cùng từ tôi nhưng ai nói thì tôi là người đó
Quy tắc: không mang giá trị cố định mà phụ thuộc vào ai là người gọi cái hàm đó

this trong object
A. quy tắc vàng, kẻ đứng trước dấu chấm = THIS
Các nhận dạng của this
Gọi qua object.ham() => this trỏ về đúng cái object đó => user.gioiThieu => this = user
arrowFunction (() =>{

}) -> kế thừa this từ scope bên ngoài => không có this riêng

> **Note:** Arrow function không có `this` riêng. `this` trong arrow function sẽ kế thừa từ scope chứa nó, nên nếu bạn dùng arrow function làm method của object thì `this` sẽ không trỏ về object đó.
